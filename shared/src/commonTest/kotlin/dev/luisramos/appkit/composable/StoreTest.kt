...
import dev.luisramos.appkit.composable.reducer.Scope
import dev.luisramos.appkit.composable.reducer.withEffect
...
class StoreTest {
        ...
        val delayedReducer = Reducer { state: Counter.State, action: Counter.Action ->
            when (action) {
                Counter.Action.Increment ->
                    dev.luisramos.appkit.composable.reducer.withEffect {
                        delay(100)
    ...
    private class AppFeature : Reducer<AppFeature.State, AppFeature.Action> {
        ...
        sealed class Action {
            ...
            companion object {
                val counter = ActionPath<Action, Counter, StoreTest.Counter.Action>(
                    ::Counter,
                    Counter::action
                )
            }
