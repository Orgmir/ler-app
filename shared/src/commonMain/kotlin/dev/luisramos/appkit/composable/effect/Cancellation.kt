...
import dev.luisramos.appkit.composable.merge
 ...
 */
fun <Output> Effect<Output>.cancellable(
    id: Any,
...
fun <T> cancelEffect(id: Any): Effect<T> = fireAndForget {
    mutex.withReentrantLock {
...
fun <T> cancelEffects(vararg ids: Any): Effect<T> =
    merge(*ids.map { cancelEffect<T>(it) }.toTypedArray())

fun <T> cancelAllRunningEffects(): Effect<T> {
    val allCancellables = cancellationCancellables.map { (key, _) -> cancelEffect<T>(key) }
    return merge(*allCancellables.toTypedArray())
}
...
interface Cancellable {
    ...
    companion object {
        operator fun invoke(block: suspend () -> Unit): Cancellable = object : Cancellable {
